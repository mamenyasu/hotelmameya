<?php
 ...
 //指定した種類の部屋の、指定されたプランの、一か月の値段レコードを取得するメソッド。
    public function getPricesRecord($room_id,$plan,$year,$month){
        ...
        try{
        $stmt=$this->pdo->prepare('SELECT * FROM pricesCalendar WHERE room_id=:room_id AND plan=:plan AND stay_date >= :startdate AND stay_date < :enddate');
...
//指定した種類の部屋の、指定されたプランの、指定された期間の値段レコードを取得するメソッド。
    public function getPricesBetween($room_id,$plan,$checkin_date,$checkout_date){
        try{
        $stmt=$this->pdo->prepare('SELECT price FROM pricesCalendar WHERE room_id=:room_id AND plan=:plan AND stay_date >= :checkin_date AND stay_date < :checkout_date');
        $stmt->bindValue(':room_id',$room_id,PDO::PARAM_INT);
        $stmt->bindValue(':plan',$plan,PDO::PARAM_STR);
